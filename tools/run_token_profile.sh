...
PROFILE_PERF="${PROFILE_PERF:-True}"
...
if [[ -n "${PROFILE_VIDEO_IDS}" ]]; then
  BASE_ARGS+=(--profile_video_ids "${PROFILE_VIDEO_IDS}")
fi

RUNS=(
  "retrieval_token_profile::"
)
...
run_one() {
  ...
  echo "ProfilePerf=${PROFILE_PERF}"
  echo "ProfileVideoIds=${PROFILE_VIDEO_IDS:-<all videos>}"
  echo "============================================================"
